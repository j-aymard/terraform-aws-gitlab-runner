...
resource "aws_launch_template" "agent_instance" {
  ...
  iam_instance_profile {
    name = "st-cicd-instance"//aws_iam_instance_profile.instance.name
  }
  ...
  network_interfaces {
    security_groups             = var.agent_security_group_ids//concat([aws_security_group.runner.id], var.extra_security_group_ids_runner_agent)
